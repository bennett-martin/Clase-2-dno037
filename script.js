...
if (5 < laHora && laHora < 12) {
    ...
    elSaludo = "καλό απόγευμα!! :)";
} else {
    elSaludo = "Καληνυχτα!!(-_-)zZZ";
}
var elColor;
function setup() {
    ...
    createA("artist (3).html", "miaumiau").parent("vinculos");
...
function portada() {
    createSpan(". βρίσκεστε στο index.html").parent("title");
    select("a:nth-child(1)").style("font-weight", "bold").style("color", "#9EA3B0");
}
function pagina() {
    createSpan(". βρίσκεστε στο page.html").parent("title");
    select("a:nth-child(2)").style("font-weight", "bold").style("color", "#9EA3B0");
}
